...
class TensorflowTransAct(tf.keras.Model):
    def __init__(self, transact_config: TransActConfig, training=False):
        ...
        if self.transact_config.concat_candidate_emb:
            transformer_in_dim = self.transact_config.action_emb_dim + self.transact_config.item_emb_dim * 2
        else:
            transformer_in_dim = self.transact_config.action_emb_dim + self.transact_config.item_emb_dim
    ...
    def call(self,
        ...
        rand_time_window_ms = tf.random.uniform(shape=(), minval=0, maxval=self.transact_config.time_window_ms,
                                                dtype=tf.int32)
        short_time_window_idx_trn = tf.math.less(
            request_time - action_time_seq, rand_time_window_ms
        )
        short_time_window_idx_eval = tf.math.less(
            request_time - action_time_seq, 0
        )
        ...
        if self.transact_config.concat_candidate_emb:
            ...
            action_pin_emb = tf.concat((action_pin_emb, item_embedding_expanded), axis=-1)

        key_padding_mask = tf.expand_dims(key_padding_mask, axis=-1)
        tfmr_out = self.transformer_encoder(encoder_inputs=action_pin_emb, attention_mask=key_padding_mask)
